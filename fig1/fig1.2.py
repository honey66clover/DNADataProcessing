...
gs1.update(top=0.95, bottom=0.13, left=0.12, right=0.42, wspace=0.05,hspace=0.05)
ax1=subplot(gs1[0:2, :])
ax2=subplot(gs1[2:, :])
gs2 = gridspec.GridSpec(7, 6)
gs2.update(top=0.95, bottom=0.13,left=0.48, right=1.07, wspace=0.1,hspace=0.05)
bx1=subplot(gs2[0:2, 0:3])
bx2=subplot(gs2[2:, 0:3])
bx3=subplot(gs2[2:, 3:4])
bx4=subplot(gs2[2:, 4:5])
...
df1['1HU/50']=df1['N(bp)']/float(100)
#print df['<HU>']
#print df['1HU/50']
x=df1['N(bp)']
#print x
p1,=ax1.plot(x[20:],df1['<HU>'][20:],'k',linewidth=0.5)
p2,=ax1.plot(x[20:],df1['1HU/50'][20:],'k:',linewidth=0.5)
ax1.set_ylabel('# HU',fontsize=8) #could be controlled by rc.font
ax1.legend([p1,p2],["circle","unconstrained"],bbox_to_anchor=(0.28, 0.51,0.5, -0.2),loc=8,fontsize=8,borderpad=0.2,labelspacing=0.1,ncol=1,handletextpad=0.,columnspacing=0.5)#not controlled by rc.font
ax1.set_ylim((0.8,5))
...
ax1.text(85.6, 4.6,'A',fontsize=8)
ax1.text(155, 4.6,'B',fontsize=8)
...
ax2.set_xlabel('DNA chain length (bp)',fontsize=8)
ax2.set_xticks([105, 126, 147])
#ax2.set_ylabel('# configurations'+r'$\mathregular{\times 10^4}$',fontsize=8)
ax2.set_ylabel('Closed configurations (%)'+r'$\mathregular{\times 10^{-6}}$',fontsize=8)
ax2.legend((p2[0],p3[0]),('2HU',r'$\mathregular{\geqslant}$'+'3HU','4HU'),bbox_to_anchor=(0.4, 0.75,0.5, -0.2),loc=8,fontsize=8)
...
with open(fileb,'rb') as rfile:
	...
	nPro=int(row[3])
        ...
        bx2.set_ylabel('Sampled configurations',fontsize=8)
        bx2.yaxis.set_label_coords(-0.02, 0.5)
...
bx1.set_ylabel(r'$\mathit{w}_\mathregular{HU}$',fontsize=8)
bx1.yaxis.set_label_coords(-0.02, 0.5)
...
x3=nProteins
y3=range(1,251)
bx3.plot(x3,y3,color='k',linewidth=0.5)
bx3.set_xlim((1,5))
bx3.set_xticks([2,4])
bx3.set_xlabel('HU #',fontsize=8)
bx3.get_yaxis().set_visible(False)
bx3.xaxis.set_ticks_position('bottom')

y4=range(1,251)
bx4.plot(pca1s,y4,color='k',linewidth=0.5)
#bx4.scatter(pca2s,y4,c='0.2',s=0.1)
bx4.set_ylim((1,250))
bx4.set_xticks([40,60])
bx4.set_xlabel(r'$\mathit{\lambda}_1$'+ur'(\u00c5)',fontsize=8)
#bx4.text(48.5,-27.2,r'$\circ$',fontsize=8)
bx4.get_yaxis().set_visible(False)
bx4.xaxis.set_ticks_position('bottom')
#bx4.set_ylabel('Sampled configurations',fontsize=8)
...
#show()
savefig('C:\Users\honey66clover\Desktop\Weijuan\DNA\HU\sample pictures2\\fig1\\fig1.2.png',dpi=600)