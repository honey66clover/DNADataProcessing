...
wfilename="2HU_spacing_probability_HU+37.dat" % locals()
with open (wfilename, 'wb') as wfile:
	for bp in range (80,150):
		rfilename="read_2protein_position_lengths-minicircle_%(bp)i_HU+37.dat" % locals()
		with open (rfilename, 'rb') as rfile:
			...
			for row in reader:
				...
				spacing[int(row[5])]+=1 #count in two HUs in one row, one configuration
				count+=2
