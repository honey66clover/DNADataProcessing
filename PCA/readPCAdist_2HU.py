...
for i in ["1","2","3"]:
	with open("PCA%(i)s2HU.dat" % locals(), 'rb') as rfile, open("PCA%(i)sdist_2HU_every200.dat" % locals(),'wb') as wfile:
		length=[0]*30
		for j in range(0,30): 
...
for i in ["1","2","3"]:
	with open("PCA%(i)s2HU.dat" % locals(), 'rb') as rfile, open("PCA%(i)sdist_2HU_every100.dat" % locals(),'wb') as wfile:
		length=[0]*60
		for j in range(0,60): 
